...
fetch('/event_booked')
    ...
    .catch(error => {
        ...
        Image_event = card$p.querySelector(".image");
        Image_event.setAttribute(
          "src",
          "assets/images/EventPageImages/Events/" + response[p].image + "/1.jpg"
        );
        console.log(Image_event);
        Price_event = card$p.querySelector(".price");
        Price_event.innerHTML =
          "<br>Price Par Personne  =  " +
          response[p].price +
          "<br> Date Debut = " +
          response[p].date +
          "<br>Place = " +
  ...
  .then((response) => {
    ...
    if (response[p])
      while (response[p]) {
        ...
        Price_event.innerHTML =
          "Your Guests Number :   " +
          response[p].guests_Number +
          "<br>Total Price :" +
          response[p].price * response[p].guests_Number +
          "<br>Date Debut = " +
          response[p].date;
        const Image_event = card$p.querySelector(".image");
        Image_event.setAttribute(
